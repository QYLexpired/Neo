...
## 特性
#### 豐富嚴謹的配色
- 預設配色：內建數十種風格各異的預設配色方案
- 自由配色：可完全自訂介面的關鍵色值，並內建數百套經典參考色庫
- 自訂主題色：支援透過選取主題色、飽和度與亮度自訂介面顏色
- 支援主題色跟隨題頭圖、跟隨系統強調色
- 新粗野主義風格配色
...
## 寫在最後
- Neo 的效能：為了實現全域的樣式改造，Neo 相比預設主題必然會消耗更多效能，主題已盡力最佳化效能（Neo+ 的 `performancetuning` 模組就是一個專門的效能最佳化模組）。但效能是一個多因素耦合的主觀感受，如果我們認為預設主題的流暢度為 1，Neo 的流暢度可能為 0.9，但如果使用了複雜的外掛、低效的程式碼片段，可能造成 0.9 × 0.9 = 0.81 的後果，此時把卡頓全部歸咎於主題是不合理的
