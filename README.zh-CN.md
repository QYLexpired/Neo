...
- 随机配色的随机池支持自由组合，并支持自由配色的配色库作为随机池
- 优化平滑光标显示行为
- 优化聚焦块指示的响应速度
- 优化列表子弹线的响应速度
- 文档标题和正文标题不再使用强调色
...
- 自由配色：支持完全自定义界面关键色值，并内置包含数百套经典配色的参考色库
- 自定义主题色：支持通过选取主题色、饱和度和亮度自定义界面颜色
- 支持主题色跟随题头图、跟随系统强调色
- 新粗野主义风格配色
...
- 彩色选区
    - 相关CSS选择器性能较差，需谨慎开启
## 写在最后
- Neo的性能：为了实现全局的样式改造，Neo相比默认主题必然会消耗更多性能，主题已经尽力优化性能（Neo+的`performancetuning`模块就是一个专门的性能优化模块）。但性能是一个多因素耦合的主观感受，如果我们认为默认主题的流畅度为1，Neo的流畅度可能为0.9，但如果使用了复杂的插件、低效的代码片段，可能造成0.9 * 0.9 =0.81的后果，此时把卡顿全部归咎为主题是不合理的
