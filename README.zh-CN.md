...
## 写在最后
- Neo的性能：为了实现全局的样式改造，Neo相比默认主题必然会消耗更多性能，主题已经尽力优化性能（Neo+的`performancetunning`模块就是一个专门的性能优化模块）。但性能是一个多因素耦合的主观感受，如果我们认为默认主题的流畅度为1，Neo的流畅度可能为0.9，但如果使用了复杂的插件、低效的代码片段，可能造成0.9 * 0.9 =0.81的后果，此时把卡顿全部归咎为主题是不合理的
